...
$url = 'https://api.mlab.com/api/1/databases/bot/collections/noomnim?apiKey='.$api_key.'';
$json = file_get_contents('https://api.mlab.com/api/1/databases/bot/collections/noomnim?apiKey='.$api_key.'&q={"question":"'.$message.'"}');
...
if (strpos($message, 'สอนนุ่มนิ่ม') !== false) {
  if (strpos($message, 'สอนนุ่มนิ่ม') !== false) {
    ...
    $message_to_reply = 'ขอบคุณนะคะที่สอนหนู';
  }
}else{
  if($isData >0){
   foreach($data as $rec){
    ...
    $message_to_reply = 'พี่ๆสามารถสอนคำต่างๆให้หนูได้โดยพิมพ์:สอนนุ่มนิ่ม[คำถาม|คำตอบ]';
  }
}
//API Url
